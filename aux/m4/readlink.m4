...
AC_DEFUN([BOSL_PROG_READLINK], [
  # check for tools to exist
  AC_CHECK_TOOLS([BOSL_READLINK],[greadlink readlink],[no])
  # handle missing
  if test "$BOSL_READLINK" == "no"; then
  ...
  AC_CACHE_CHECK([whether readlink supports -f],
    ...
    [if test "BOSL_READLINK" -f 2>&1 < /dev/null | grep "missing operand" > /dev/null; then
